...
def path_length(graph, path):
    ...
    if len(path)>1:
        for i in range (len(path)-1):
            pathlength+=graph.get_edge(path[i],path[i+1]).length
    return pathlength        
...
def extensions(graph, path):
    ...
    for node in sorted(neighbors):
        if node not in path:
            nodes+=[node]
    newpaths=[(path+[node]) for node in nodes]
    #for npath in newpaths:
    #    if has_loops(npath):
    #        newpaths.remove(npath)
    return newpaths        
...
def plength(graph, goalNode, paths):
    paths = sorted(paths)
    return sorted(paths, key= lambda path:path_length(graph, path))

def heur(graph, goalNode, paths):
    paths= sorted(paths)
    return sorted(paths, key= lambda path:graph.get_heuristic_value(path[-1],goalNode))

def potlength(graph, goalNode, paths):
    paths = sorted(paths)
    potval=lambda p:(graph.get_heuristic_value(p[-1],goalNode)+path_length(graph,p))
    return sorted(paths, key=potval)
    
generic_dfs = [alphabetical, True, do_nothing_fn, False]

generic_bfs = [alphabetical, False, do_nothing_fn, True]

generic_hill_climbing = [heur, True, do_nothing_fn, False]

generic_best_first = [do_nothing_fn, True, heur, False]

generic_branch_and_bound = [alphabetical, False, plength, False]

generic_branch_and_bound_with_heuristic = [alphabetical, False, potlength, False]

generic_branch_and_bound_with_extended_set = [alphabetical, False, plength, True]

generic_a_star = [do_nothing_fn, False, potlength, True]
...
def beamsort(graph, goalNode, paths, beam_width):
...
generic_beam = [do_nothing_fn, False, beamsort, False]
...
def is_consistent(graph, goalNode):
    ...
    for n1 in nds:
        for n2 in nds:
            heurdiff=abs(graph.get_heuristic_value(n1, goalNode)-graph.get_heuristic_value(n2, goalNode))
            pl=path_length(graph,generic_search(*generic_branch_and_bound_with_extended_set)(graph,n1,n2))
            if heurdiff>pl:
                flag=1
    if flag == 1:
        return False
...
[h2_S, h2_A, h2_B, h2_C, h2_G] = [8, 0, 10, 7, 0]
...
[h3_S, h3_A, h3_B, h3_C, h3_G] = [8, 8, 1, 3, 0]
...
[h4_S, h4_A, h4_B, h4_C, h4_G] = [8, 0, 10, 7, 0]
