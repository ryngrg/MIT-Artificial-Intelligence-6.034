...
def expression_depth(expr):
    ...
    if not isinstance(expr,list):
        ...
        depth=1+max(expression_depth(e) for e in expr)
    return depth    
...
def tally_letters(string):
    ...
    lettertally={}
    for i in range (0,len(string)):
        if string[i]in lettertally:
            lettertally[string[i]]+=1
        else:
            lettertally[string[i]]=1
    return lettertally    
...
def furthest_right_point(list_of_points):
    ...
    p=max(list_of_points,key=lambda point :point.getX())
    return p
...
NAME = "Aaryan Garg"
COLLABORATORS = ""
HOW_MANY_HOURS_THIS_LAB_TOOK = 1.9834578364
SUGGESTIONS = None
